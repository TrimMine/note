    ...
    $('.modal-content').on('click','#save',function(){
        ...
        $.post("{:U('ArticleClass/add_class')}",{class:data},function(data){
            if (data.status == 'success') {
                ...
                // 在该样式的最后一个标签后添加
                $('.btn-outline:last').after("<button type='button' data-id='"+data.id+"' class='btn btn-outline btn-danger'"+ 
                    "style='margin-left:5px;margin-top:5px'>"+
                        "<span>"+data.name.class+"</span>"+
                         ...
                         // 鼠标移入样式
                        $('.btn-outline').on('mouseover',function(){
                            // 显示删除标签
                        ...
                        // 鼠标移除恢复样式
                         $('.btn-outline').on('mouseout',function(){
                            // 显示删除标签
