...
class Ajax extends Backend
{
    ...
    public function excelReader($file)
    {
        ...
        if (@fopen($file, 'r')) {
            ...
            // $objWorksheet = $objPHPExcel->getActiveSheet(); // 不兼容linux excel, 使用下面方式
            ...
            $users = new \app\admin\controller\user\User();
...
//                    }
                    ...
                    //组装数据到data
                    $data [] = $users->import_data($v);
                    //Your code here...
                }
                //插入数据
                $res = UsersModel::insertAll($data);
                if (!$res){
                    return false;
                }
            } else {
    ...
    public function upload()
    {
        ...
        if ($splInfo) {
            ...
            $params = array(
                ...
                'storage' => 'local',
                'sha1' => $sha1,
                'raw_name' => $fileInfo['name'],
            );
            $attachment = model("attachment");
            $attachment->data(array_filter($params));
            $attachment->save();
            \think\Hook::listen("upload_after", $attachment);
            //添加的内容 判断是否是用户列表导入数据 且类型为xls
            if (strpos($param, $contorller_url) && $suffix == 'xls') {
                  $this->excelReader($params['url']);
            }
