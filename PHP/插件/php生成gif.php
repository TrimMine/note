...
* 
*/
class GIFEncoderController extends Controller
{
   ...
   public function make()
    {
        ...
        while($count != 10){ 
            if ($count % 2 == 1) {
                $img = imagecreatefromjpeg('Public/1.jpg');         //$img 每一帧的图片
            } else {
                $img = imagecreatefromjpeg('Public/2.jpg');
            }
            imagegif($img);
            imagedestroy($img); 
            $imagedata[] = ob_get_contents();        //$imagedata gif图片组
            ob_clean(); 
            $count++;
        }
        $gif = new GIFEncoder1Controller($imagedata,0,0,2,0,0,1,"bin"); 
        ob_start(); 
     ...
     public function makeGif()
     {
        ...
        */
        if ( $dh = opendir ( "Public/images/" ) ) { 
          while ( false !== ( $dat = readdir ( $dh ) ) ) { 
            if ( $dat != "." && $dat != ".." ) { 
              $frames [ ] = "Public/images/$dat"; 
              $framed [ ] = 5; 
            } 
          } 
          closedir ( $dh ); 
